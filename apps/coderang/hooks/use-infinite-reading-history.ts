...
import useFeedInfiniteScroll from './feed/use-feed-infinite-scroll';
...
function useInfiniteReadingHistory({
  ...
  const { isLoading, isFetchingNextPage, hasNextPage, data, fetchNextPage } = queryResult;
  ...
  const hasData = data?.pages?.some(page => page.readHistory.edges.length > 0);
