...
} from '../graphql/feed-settings';
import { Source } from '../graphql/sources';
import { getFeedSettingsQueryKey } from './use-feed-settings';
import { RequestKey, generateQueryKey } from '../lib/query';

export const getSearchTagsQueryKey = (query: string): string[] => ['searchTags', query];
...
type UpdateAdvancedSettings = (params: AdvancedSettingsMutationProps) => Promise<unknown>;
...
type ManipulateSourceFunc = (feedSettings: FeedSettings, source: Source) => FeedSettings;
...
export default function useMutateFilters(user?: LoggedUser): ReturnType {
  ...
  const updateFeedFilters = useCallback(
    ({ advancedSettings, ...filters }: FeedSettings) => {
      const { blockedTags = [], excludeSources = [], includeTags = [] } = filters ?? {};
  ...
  return useMemo(
    () => ({
      ...
      unfollowSource: shouldFilterLocally ? onUnfollowSource : unfollowSourceRemote,
