...
import { AnalyticsContextData } from './use-analytics-context-data';
import { AnalyticsEvent } from './use-analytics-queue';

const ACTIVE_STATES = ['active', 'passive'];

const isActiveState = (state: string): boolean => ACTIVE_STATES.indexOf(state) > -1;
...
export default function useTrackLifecycleEvents(
  ...
  useEffect(() => {
    ...
    const callback = (event: CustomEvent) => lifecycleCallbackRef.current(event);
