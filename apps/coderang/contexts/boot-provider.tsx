...
import { storageWrapper as storage } from '../lib/storage-wrapper';
import { useRefreshToken } from '../hooks/use-refresh-token';
import { BOOT_LOCAL_KEY, BOOT_QUERY_KEY } from './common';
import { useHostStatus } from '../hooks/use-host-permission-status';
import { checkIsExtension } from '../lib/func';
import { AlertContextProvider } from './alert-context';
import { AnalyticsContextProvider } from './analytics-context';
import { GrowthBookProvider } from '@/components/growth-book-provider';
...
export const BootDataProvider = ({
  ...
  const [lastAppliedChange, setLastAppliedChange] = useState<Partial<BootCacheData>>();
  const [initialLoad, setInitialLoad] = useState<boolean>(null);
  const loadedFromCache = !!cachedBootData;
  const { user, settings, alerts, notifications, squads } = cachedBootData || {};
  ...
  const setBootData = (updatedBootData: Partial<BootCacheData>, update = true) => {
  ...
  const updateUser = useCallback(
    async (newUser: LoggedUser | AnonymousUser) => {
      ...
      setCachedBootData(updated);
      await queryClient.invalidateQueries(generateQueryKey(RequestKey.Profile, newUser));
    },
