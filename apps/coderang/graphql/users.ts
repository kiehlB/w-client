...
import { SHARED_POST_INFO_FRAGMENT, USER_SHORT_INFO_FRAGMENT } from './fragments';
...
export const USER_TOOLTIP_CONTENT_QUERY = gql`
  query UserTooltipContent($id: ID!, $version: Int, $requestUserInfo: Boolean!) {
...
export const USER_READING_HISTORY_QUERY = gql`
  query UserReadingHistory(
    ...
    userReadingRankHistory(id: $id, version: $version, after: $after, before: $before) {
      rank
      count
    }
...
export const SEARCH_READING_HISTORY_QUERY = gql`
  ...
  query SearchReadingHistory($first: Int, $after: String, $query: String!) {
    readHistory: searchReadingHistory(first: $first, after: $after, query: $query) {
...
export const SUBSCRIBE_PERSONALIZED_DIGEST_MUTATION = gql`
  mutation SubscribePersonalizedDigest($hour: Int, $day: Int, $type: DigestType) {
