import { MessageEventData } from '../hooks/use-event-listener';
...
export type KratosMethod = 'link' | 'link_recovery' | 'password' | 'profile' | 'code';
...
export const checkKratosEmail = async (email: string): Promise<KratosEmailData> => {
...
export const getKratosProviders = async (flow?: string): Promise<KratosProviderData> => {
...
>({
  ...
  const hasError = json.ui?.messages?.some(({ type }) => type === MessageType.Error);
...
export const getVerificationSession = async (): Promise<VerificationSession> => {
