...
import { ReadHistoryInfiniteData } from '../hooks/use-infinite-reading-history';
...
export const generateStorageKey = (key: RequestKey, ...params: string[]): string =>
...
export type HasConnection<TEntity, TKey extends keyof TEntity = keyof TEntity> = Partial<
  Record<TKey, Connection<unknown>>
>;
...
>({
  ...
  return client.setQueryData<TReturn>(queryKey, data => {
    ...
    const updated = { ...data };
    const item = updated.pages[page][prop].edges[edge].node as EmptyObjectLiteral;
...
export const updateCachedPage = (
  ...
  queryClient.setQueryData<InfiniteData<FeedData>>(feedQueryKey, currentData => {
