...
export type ValidateRegistrationParams = KratosFormParams<RegistrationParameters>;
...
): InitializationNode => nodes?.find(({ attributes }) => attributes.name === key);

export const getNodeValue = (key: string, nodes: InitializationNode[]): string =>
