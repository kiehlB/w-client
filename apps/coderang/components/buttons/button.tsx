import React, { forwardRef, HTMLAttributes, ReactElement, ReactNode, Ref } from 'react';
...
function ButtonComponent<TagName extends AllowedTags>(
  ...
  return (
    <Tag
      ...
      {icon &&
        [ButtonIconPosition.Left, ButtonIconPosition.Top].includes(iconPosition) &&
        getIconWithSize(icon)}
      {loading ? <span className="invisible">{children}</span> : children}
      {icon && iconPosition === ButtonIconPosition.Right && getIconWithSize(icon)}
      {loading && (
        <Loader data-testid="buttonLoader" className="btn-loader absolute m-auto" />
      )}
    </Tag>
  );
}
