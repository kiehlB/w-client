...
import {ElementState, useElementState} from "@w/use-element-state";
...
const arrowVariants: Record<ArrowIconProps["direction"], Record<ElementState, Variant>> = {
  down: {
    initial: {y: 0},
    hover: {y: 4},
    focus: {
...
function ArrowButtonContent({
  ...
  return (
    <>
      {children && (direction === "right" || direction === "up" || direction === "top-right") ? (
...
function NextLink({
  ...
  children,
  onClick,
}: {href: LinkProps["href"]} & Pick<ArrowLinkProps, "className" | "children" | "onClick">) {
  const [ref, state] = useElementState();
  const shouldReduceMotion = useReducedMotion();
